...
export const tagsSlice = createSlice({
  ...
  initialState,
  reducers: {
    addTag: (state, action: PayloadAction<{ containerId: string; tag: Tag }>) => {
    ...
    removeTag: (state, action: PayloadAction<{ containerId: string; tagId: string }>) => {
      const { containerId, tagId } = action.payload
      if (state[containerId]) {
        state[containerId] = state[containerId].filter(tag => tag.id !== tagId)
      }
    },
