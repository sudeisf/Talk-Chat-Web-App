...
export function TagsFilterSelect({
  ...
  // Get selected tags from Redux or use external prop
  const dispatch = useAppDispatch();
  const reduxSelectedTags = useAppSelector((state: any) => state.tags?.[selectedTagsContainerId] || []);
  ...
  const handleTagSelect = (tag: Tag) => {
    const isSelected = selectedTags.some((selected: { id: string }) => selected.id === tag.id)
    ...
    if (externalOnChange) {
      ...
      if (isSelected) {
        dispatch(removeTag({ containerId: selectedTagsContainerId, tagId: tag.id }))
      } else {
            ...
            <CommandList>
              ...
              <CommandGroup>
                {filteredTags.map((tag) => {
                  const isSelected = selectedTags.some((selected: { id: string }) => selected.id === tag.id)
                  return (
                    <CommandItem key={tag.id} value={tag.label} onSelect={() => handleTagSelect(tag)}>
